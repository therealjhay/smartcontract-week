...
*   **Storage (Permanent):** If declared outside a function (state variables), `structs`, `arrays`, and `mappings` are stored on the blockchain. This data persists forever.
*   **Memory (Temporary):** If declared inside a function (local variables), `structs` and `arrays` are stored in memory. This data is wiped clean after the function finishes.
